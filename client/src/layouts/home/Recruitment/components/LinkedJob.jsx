  ...
  return (
    <div>
      ...
      {!loading && crawlData?.length > 0 && (
        <ul className='relative jobs-search__results-list flex flex-col gap-8'>
          {crawlData?.map((c, index) => {
            return (
              <li
                className='py-2 px-4 rounded-sm border border-neutral-300 dark:border-neutral-800 flex flex-col gap-2'
                key={index}
